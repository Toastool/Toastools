...
<script>
	...
	function stringFormat(p_val) {
		if (p_val < 10) return (p_val = '0' + p_val);
		else return p_val;
	}
	...
	function startChange() {
		var start = $('#start').val();
		var end = $('#end').val();
		if (start.substring(3, 5) == '00') end = start.substring(0, 2) + ':30';
		else end = parseInt(start.substring(0, 2)) + 1 + ':00';
	...
	function insertModalOpen(arg) {
		...
		if (g_arg.event != undefined) {
			...
			$('.insertModal #start').val(
				stringFormat(g_arg.event.start.getHours()) +
					':' +
					stringFormat(g_arg.event.start.getMinutes())
			);
			$('.insertModal #end').val(
				stringFormat(g_arg.event.end.getHours()) +
					':' +
					stringFormat(g_arg.event.end.getMinutes())
			);
	...
	function approvalSch(modal, arg) {
		...
		if (confirm('일정을 승인하시겠습니까?')) {
			...
			if ($('.' + modal + ' #title').val().length > 100) {
				alert(
					'제목은 100자를 넘을 수 없습니다 현재 ' +
						$('.' + modal + ' #title').val().length +
						'자'
				);
				return;
			}
			...
			if (arg.event != undefined) {
				...
				var m_start = new Date(
					arg.event.startStr.substr(0, 4),
					arg.event.startStr.substr(5, 2) - 1,
					arg.event.startStr.substr(8, 2)
				);
				//종료일자 ex)2021.09.30
				var m_end = new Date(
					arg.event.endStr.substr(0, 4),
					arg.event.endStr.substr(5, 2) - 1,
					arg.event.endStr.substr(8, 2)
				);
				...
				if ($('.insertModal input[name="allDay"]:checked').val() != 'true') {
					...
					if (
						m_end_com.getFullYear() +
							'' +
							stringFormat(m_end_com.getMonth()) +
							'' +
							stringFormat(m_end_com.getDate()) ==
						m_first.getFullYear() +
							'' +
							stringFormat(m_first.getMonth()) +
							'' +
							stringFormat(m_first.getDate())
					) {
					...
					data = {
						...
						startdt:
							arg.event.startStr.substr(0, 10) +
							'T' +
							$('.' + modal + ' #start').val(),
...
// 							);
// 							arg.event.setEnd(
// 								new Date(m_end_dt + 'T' + $('.' + modal + ' #end').val())
// 							);
// 							arg.event.setExtendedProp('allowyn', '1');
// 						}
	...
	function insertSch(modal, arg) {
		...
		if (arg.event != undefined) {
			...
			if ($('.insertModal input[name="allDay"]:checked').val() != 'true') {
				...
				m_date = m_end.getDate();
				m_end_dt =
					m_end.getFullYear() + '-' + stringFormat(m_month) + '-' + stringFormat(m_date);
				data = {
					id: arg.event.id,
					title: $('.' + modal + ' #title').val(),
					startdt:
						arg.event.startStr.substr(0, 10) + 'T' + $('.' + modal + ' #start').val(),
					enddt: m_end_dt + 'T' + $('.' + modal + ' #end').val(),
					allday: $('.' + modal + ' input[name="allDay"]:checked').val(),
					allowyn: '0',
					comments: '',
			...
			if ($('.insertModal input[name="allDay"]:checked').val() != 'true') {
				var m_start = new Date(
					arg.startStr.substr(0, 4),
					arg.startStr.substr(5, 2) - 1,
					arg.startStr.substr(8, 2)
				);
				var m_end = new Date(
					arg.endStr.substr(0, 4),
					arg.endStr.substr(5, 2) - 1,
					arg.endStr.substr(8, 2)
				);
				var m_month = m_end.getMonth() + 1;
				...
				if (arg.endStr.length > 10) {
					...
					arg.endStr =
						m_end.getFullYear() +
						'-' +
						stringFormat(m_end.getMonth() + 1) +
						'-' +
						stringFormat(m_end.getDate());
				}
