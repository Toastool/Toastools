...
<html>
	...
	<body>
		...
		$hit = mysqli_fetch_array(mq("select * from Toast_board where idx ='".$bno."'"));
		$hit = $hit['hit'] + 1;
		$fet = mq("update Toast_board set hit = '".$hit."' where idx = '".$bno."'");
		$sql = mq("select * from Toast_board where idx='".$bno."'"); /* 받아온 idx값을 선택 */
...
<div class="reply_view">
	<h3>댓글목록</h3>
		<?php
			$sql3 = mq("select * from Toast_boardReply where con_num='".$bno."' order by idx desc");
	...
	<div class="dap_ins">
		<form action="reply_ok.php?idx=<?php echo $bno; ?>" method="post">
			...
			<div style="margin-top:10px; ">
				...
				<button id="rep_bt" class="re_bt">댓글<br>작성</button>
			</div>
		</form>
	</div>
</div><!--- 댓글 불러오기 끝 -->
